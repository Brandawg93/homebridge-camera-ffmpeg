import { API, APIEvent, AudioRecordingCodecType, CameraController, CameraRecordingConfiguration, CameraRecordingDelegate, H264Level, H264Profile, HAP, HDSProtocolSpecificErrorReason, RecordingPacket } from 'homebridge';
  ...
  async * handleFragmentsRequests(configuration: CameraRecordingConfiguration): AsyncGenerator<Buffer, void, unknown> {
    ...
    const audioArgs: Array<string> = [
      ...
      ...(configuration.audioCodec.type === AudioRecordingCodecType.AAC_LC ?
        ['-profile:a', 'aac_low'] :
        ['-profile:a', 'aac_eld']),
      '-ar', `${configuration.audioCodec.samplerate}k`,
